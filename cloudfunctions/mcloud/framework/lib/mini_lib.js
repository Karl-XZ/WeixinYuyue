...
async function sendMiniOnceTempMsg(body, key = '') {
	...
	try {
		...
		console.log('[sendMiniOnceTempMsg] key=' + key + ', touser=' + body.touser + ', templateId=' + body.templateId);
		await cloud.openapi.subscribeMessage.send(body);
		console.log('[sendMiniOnceTempMsg] 消息已发送成功, key=' + key);
	} catch (err) {
		console.error('[sendMiniOnceTempMsg] 发送失败 key=' + key + ', error:', err.message || err);
