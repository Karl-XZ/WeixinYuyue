...
class NewsService extends BaseProjectService {
	...
	async viewNews(id) {
		...
		await this._newsImageService.formatNews(news);
	...
	async getNewsList({
		...
		const result = await NewsModel.getList(where, fields, orderBy, page, size, isTotal, oldTotal);
		for (let k = 0; k < result.list.length; k++) {
			await this._newsImageService.formatNews(result.list[k]);
		}
		return result;
	}  
